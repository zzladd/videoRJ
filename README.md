...
| 剪辑台 | 拖拽片段到时间线自由拼接：排序、裁剪起止点、逐段转场与字幕，所见即所得提交渲染 |
...
4. **三种剪辑方式**：
   - **脚本智能匹配**：素材库勾选多个素材 → 「渲染所选素材」→ 选脚本，系统在所选范围内按镜头关键词匹配片段，带字幕
   - **自动混剪**：同上但不选脚本，所选素材轮流取片段、叠化转场、保留原声，指定成片时长即可
   - **剪辑台（手动拖拽）**：「剪辑台」页签，把各素材的场景片段**拖拽**到时间线自由拼接，支持拖拽排序、裁剪起止点、逐段设置转场（硬切/叠化/黑场/滑动/划像）、逐段填写字幕文案
5. **预览/下载**：在"渲染任务"页在线预览、下载 mp4 与 SRT

> 对应 API：`POST /api/render`（多素材 + 可选脚本）、`POST /api/render/timeline`（剪辑台自定义时间线）、
> `POST /api/scripts/{id}/render`（默认全库素材匹配）。
